# sw2.rsc — containerlab CHR config.
#
...
#                ether4 = sw1 (inter-switch), ether5 = uplink (vlan1).
...
/ip address add address=192.168.90.1/24 interface=vlan1
...
/routing bgp instance add name=default as=65002 router-id=192.168.90.1 multipath=4
/routing bgp template add name=ebgp-default as=65002 afi=ip,ipv6 use-bfd=yes
...
/ip firewall address-list add list=bgp-networks address=192.168.90.0/24
...
# Peer: inter-switch link to sw1 on ether4
/routing bgp connection add name=peer-sw1 instance=default templates=ebgp-default local.address=ether4 .role=ebgp output.network=bgp-networks
